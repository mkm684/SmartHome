...
public class Window implements Device {
	...
	Window(String name, GreetingServer server, String room_name){
		this.name = name;
		this.room_name = room_name;
		on_or_off = false;
		this.server = server;
		try {
			server.out.writeUTF(room_name+"/"+name+"W"+"created");
		} catch (IOException e) {
	...
	public void turn_on(){
		on_or_off = true;
		System.out.println(name + "window is on");
		try {
	...
	public void turn_off(){
		on_or_off = false;
		System.out.println(name +"window is off");
		try {
